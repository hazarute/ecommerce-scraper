...
def main():
    ...
    if secim and secim.isdigit() and 1 <= int(secim) <= len(site_keys):
        ...
        # Otomatik site tespiti
        auto_site_key = None
        for key in site_keys:
            if config[key]["url"].replace("https://","").replace("http://","") in url:
                auto_site_key = key
                break
        if auto_site_key:
            print(f"[Otomatik] '{url}' adresi için '{auto_site_key.title()}' selectorları seçildi.")
            site_conf = config[auto_site_key]
            selectors = site_conf["selectors"]
            wait_time = site_conf.get("wait_time", 5)
        else:
